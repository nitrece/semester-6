...
/* Identifying the Speaker */
	for ( i=0; i<Number_Of_Speakers; i++ ) { /* Total Number of trained Speakers */
		distance = 0.0; /* Initialize the distance to be Zero */
  		for ( j=0; j<Number_Of_Coefficients; j++ ) { /* Total Number of Stored Co-efficients
	 		     									  * of the training Vector */
/* Compute Euclidean Distance */
  			distance = distance + ((mfcc_vector[j]-training_vector[i][j])*(mfcc_vector[j]-training_vector[i][j]));
		}
/* Identify the speaker with least distance */
		if ( distance < ref_distance ) { /* check if the distance is less than 
		    								* previous computed distances */
